...
interface VehicleEmailData {
  vehicle: Pick<Vehicle, 'id' | 'radioCallName' | 'licensePlate'>;
...
export const getRentalConfirmationEmail = (data: VehicleEmailData) => ({
  to: data.user.email,
  subject: `Fahrzeugausleihe bestätigt: ${data.vehicle.radioCallName}`,
  html: `
    ...
    <ul>
      <li><strong>Fahrzeug:</strong> ${data.vehicle.radioCallName} (${data.vehicle.licensePlate})</li>
...
export const getRentalCancellationEmail = (data: VehicleEmailData) => ({
  to: data.user.email,
  subject: `Fahrzeugausleihe storniert: ${data.vehicle.radioCallName}`,
  html: `
